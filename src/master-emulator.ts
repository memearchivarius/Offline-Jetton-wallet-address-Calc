...
async function main() {
    ...
    // First we need to deploy the contract is sandbox environment
    ...
    // Open contract, so sandbox can send messages to it
    const hamsterAddress = Address.parse("EQAJ8uWd7EBqsmpSWaRdf_I-8R8-XHwh3gsNKhy-UrdrPcUo")
    const openedContract = blockchain.openContract(
        new MyContract(hamsterAddress, 
            {code: contractCode, data: contractData})
    );

    // Instead of deploying contract, we can set shard account directly
    await blockchain.setShardAccount(hamsterAddress, createShardAccount({
